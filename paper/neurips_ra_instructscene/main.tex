...
Our experiments use the original InstructScene bedroom, living-room, and dining-room validation splits. The paper-facing collision-gated \fp{} repair improves realized relation accuracy from 0.739 to 0.816 in bedrooms, 0.551 to 0.639 in living rooms, and 0.595 to 0.673 in dining rooms, corresponding to gains of 7.8, 8.8, and 7.8 absolute points. Across three seeds, collision-gated relation gains remain positive for all room types, while mean mesh-collision pair deltas become negative. Real 3D-FUTURE mesh render diagnostics show selected \fp{} examples improve relation counts without increasing mesh pairs and with high before/after structural similarity.
...
\subsection{Main Collision-Gated Results}

Table~\ref{tab:main} reports the paper-facing collision-gated \fp{} method. The gate falls back to the baseline layout whenever repair would increase mesh collision. Gated relation gains remain positive in every room, and gated mesh pair rates remain below the baseline.

\begin{table}[t]
\centering
\caption{Main collision-gated \fp{} results. The gate keeps repaired layouts only when FCL mesh collision pairs do not increase.}
\label{tab:main}
\small
...
\subsection{Ungated Floor-Prior Ablation}

Table~\ref{tab:ungated} reports the ungated \fp{} ablation. It produces larger relation gains of 10.2--14.3 absolute points, but its mesh pair deltas are positive. These values characterize the effect of ungated repair and are not the Eurographics main result.

\begin{table}[t]
\centering
\caption{Ungated \fp{} ablation. Accuracy is exact realized selected-relation accuracy. Movement is the average floor-plane movement for repaired pairs.}
\label{tab:ungated}
...
Table~\ref{tab:multiseed} summarizes three-seed results for the frozen \fp{} configuration. Ungated relation gains are stable and positive across all rooms. After collision gating, relation gains remain positive, and the mean mesh delta is negative for every room.
...
We presented \method, a training-free verifier-repair layer for instruction-guided 3D indoor scene generation. The method addresses a graph-to-layout grounding gap in InstructScene by parsing explicit spatial relations, verifying final layout geometry, and applying bounded floor-plane repairs that preserve object heights. On existing InstructScene validation splits, the paper-facing collision-gated \fp{} repair retains positive relation gains while controlling FCL mesh collision; ungated Floor-Prior and Direct Repair remain internal ablations. These results make relation-consistency repair a practical and reproducible direction for improving pretrained 3D scene generators.
