...
Our experiments use the original InstructScene bedroom, living-room, and dining-room validation splits. The main \fp{} repair improves realized relation accuracy from 0.739 to 0.841 in bedrooms, 0.551 to 0.694 in living rooms, and 0.595 to 0.729 in dining rooms. Across three seeds, relation gains remain positive for all room types. After collision gating, mean relation gains remain +0.080, +0.070, and +0.087 for bedrooms, living rooms, and dining rooms, respectively, while mean mesh-collision pair deltas become negative. Real 3D-FUTURE mesh render diagnostics show selected \fp{} examples improve relation counts without increasing mesh pairs and with high before/after structural similarity.
...
Table~\ref{tab:gated} reports the conservative collision-gated \fp{} variant. The gate reduces relation gains relative to ungated repair because it falls back on scenes where mesh collision would increase. Nevertheless, gated relation gains remain positive in every room, and gated mesh pair rates fall below baseline. This is the safest variant for mesh-validity claims.
