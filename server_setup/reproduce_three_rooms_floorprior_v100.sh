...
LOGDIR=$ROOT/logs
mkdir -p "$LOGDIR"
LOG=$LOGDIR/reproduce_three_rooms_floorprior_v100_${STAMP}.log
exec > >(tee "$LOG") 2>&1
...
run_room() {
  ...
  local suffix="v100_floorprior_max1p8_objectmatch_${STAMP}"
...
print('SUMMARY', room, 'scenes', len(d.get('per_scene', [])), 'baseline', base, 'repair', rep, 'gain', rep-base, 'parser_recall', s.get('parser_relation_recall'), 'avg_move', s.get('avg_repair_movement'), 'layout_overlap', s.get('layout_overlap_ratio'), 'repair_overlap', s.get('repair_overlap_ratio'))
PY
}
run_room bedroom 1999
run_room livingroom 1459
run_room diningroom 1239
echo "[three_rooms_floorprior] PASS log=$LOG"
